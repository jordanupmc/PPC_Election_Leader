...
class Node (val id:Int, val terminaux:List[Terminal]) extends Actor {
     ...
     def receive = {
          ...
          case BeatLeader (nodeId) => {
               displayActor ! Message("I am alive "+ nodeId + " and I am the LEADER !")
               terminaux.foreach(n => {
                    if(n.id != id ){
                         val remote = context.actorSelection("akka.tcp://LeaderSystem" + n.id + "@" + n.ip + ":" + n.port + "/user/Node")
                         remote ! IsAliveLeader (id)
                    }
               })
          }

          case Beat (nodeId) => {
               displayActor ! Message("I am alive "+ nodeId)
               terminaux.foreach(n => {
                    if(n.id != id ){
                         val remote = context.actorSelection("akka.tcp://LeaderSystem" + n.id + "@" + n.ip + ":" + n.port + "/user/Node")
                         remote ! IsAlive (id)
          ...
          case IsAlive (id) => {
               displayActor ! Message (id + " is alive" )
               checkerActor ! IsAlive(id)
          }

          case IsAliveLeader (id) => {
               displayActor ! Message ("The LEADER "+ id + " is alive" )
               checkerActor ! IsAliveLeader(id)
          }

          // Message indiquant que le leader a change
          case LeaderChanged (nodeId) => {
               displayActor ! Message ("The LEADER CHANGED it is now "+ nodeId )
               beatActor ! LeaderChanged(nodeId)
               //On reinitialise les variables de electionActor en vue d'une prochaine election
